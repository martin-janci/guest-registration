  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-fg-subtle">{t('detail.eyebrow')}</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight text-fg font-mono">{inv.invoiceNumber}</h1>
          <p className="mt-1 text-sm text-fg-muted">
            {inv.clientName} · {t('detail.issued', { date: fmtDate(inv.issueDate) })}
            {inv.dueDate && <> · {t('detail.due', { date: fmtDate(inv.dueDate) })}</>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Pill tone={statusTone(inv.status)}>{t(`status.${inv.status}`)}</Pill>
          <Link href={`/admin/invoices/${inv.id}/edit`}><Button variant="secondary">{tc('edit')}</Button></Link>
        </div>
      </header>

      <section className="rounded-lg border border-border bg-surface p-6 shadow-xs">
        <h2 className="text-sm font-semibold text-fg">{t('detail.clientHeading')}</h2>
        <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          <dt className="text-fg-muted">{t('detail.clientName')}</dt>
          <dd className="text-fg">{inv.clientName}</dd>
          <dt className="text-fg-muted">{t('detail.clientEmail')}</dt>
          <dd className="text-fg">{inv.clientEmail ?? '—'}</dd>
          <dt className="text-fg-muted">{t('detail.clientVat')}</dt>
          <dd className="text-fg">{inv.clientVatNumber ?? '—'}</dd>
          <dt className="text-fg-muted">{t('detail.clientAddress')}</dt>
      ...
      <section className="overflow-hidden rounded-lg border border-border bg-surface shadow-xs">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 className="text-sm font-semibold text-fg">{t('detail.lineItemsHeading')}</h2>
          <span className="text-xs text-fg-muted">{t('detail.itemCount', { count: inv.items.length })}</span>
        </div>
        {inv.items.length === 0 && (
          <p className="px-4 py-6 text-center text-sm text-fg-muted">{t('detail.noItems')}</p>
        )}
        {inv.items.map((it) => (
          <ItemRow
            key={it.id}
      ...
      <section className="flex justify-end rounded-lg border border-border bg-surface p-4 shadow-xs">
        <dl className="flex flex-col gap-1 text-sm">
          <div className="flex items-baseline gap-6">
            <dt className="w-32 text-right text-fg-muted">{t('detail.subtotal')}</dt>
            <dd className="w-32 text-right tabular-nums text-fg">{formatMoney(inv.subtotal.toFixed(2), inv.currency)}</dd>
          </div>
          <div className="flex items-baseline gap-6">
            <dt className="w-32 text-right text-fg-muted">{t('detail.vat')}</dt>
          ...
          <div className="flex items-baseline gap-6 border-t border-border pt-1">
            <dt className="w-32 text-right font-semibold text-fg">{t('detail.total')}</dt>
      ...
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-surface p-4 shadow-xs">
        <div className="flex flex-wrap gap-2">
          <a href={`/admin/invoices/${inv.id}/pdf`} target="_blank" rel="noopener noreferrer">
            <Button variant="secondary">{t('actions.downloadPdf')}</Button>
          </a>
          {isDraft && (
          ...
          {canMarkPaid && (
            <form action={`/admin/invoices/${inv.id}/mark-paid`} method="post">
              <Button type="submit" variant="secondary">{t('actions.markPaid')}</Button>
            </form>
          )}
        </div>
        {inv.items.length === 0 && (isDraft || isSent) && (
          <p className="text-xs text-warning-700">{t('detail.warnNoItems')}</p>
        )}
      </section>

      {inv.notes && (
        <section className="rounded-lg border border-border bg-surface p-6 shadow-xs">
          <h2 className="text-sm font-semibold text-fg">{t('detail.notesHeading')}</h2>
